...
def enviar_camadas_para_api():
    ...
    try:
        resposta = requests.post(url, json=camadas)
...
def desenhar_perfil(event=None):
    ...
    for i, camada in enumerate(camadas):
        ...
        # Bordas
        canvas.create_line(50, y_atual, 50, y_atual + altura_px, fill="black")  # esquerda
        canvas.create_line(canvas_largura, y_atual, canvas_largura, y_atual + altura_px, fill="black")  # direita
        canvas.create_line(50, y_atual, canvas_largura, y_atual, fill="black")  # superior
        ...
        if not proxima or not (
            ...
            canvas.create_line(50, y_atual + altura_px, canvas_largura, y_atual + altura_px, fill="black")  # inferior

        # Descrição e Peso (γ) no centro (se for diferente da anterior)
        if not (mesma_desc and mesmo_peso):
            texto = f'{camada["Descrição"]}\nγ: {camada["Peso"]} kN/m³'
            canvas.create_text(
                (canvas_largura + 50) / 2, y_atual + altura_px / 2,
                text=texto, font=("Arial", 8), fill="black", justify="center"
            )

        # Profundidade à esquerda
        canvas.create_text(
            30, y_atual + altura_px / 2,
            text=f"{profundidade_acumulada:.2f} m",
            font=("Arial", 8), fill="black"
        )

        # NSPT dentro da camada (alinhado à direita)
        canvas.create_text(
            canvas_largura - 10, y_atual + altura_px - 10,
            text=f"NSPT: {camada['NSPT']}",
            font=("Arial", 8), fill="black", anchor="e"
        )
...
entry_peso = criar_linha_input("Peso específico (kN/m³):", var_peso)
